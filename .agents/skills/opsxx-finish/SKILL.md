...
3. **Reconcile tasks**
   ...
   Otherwise, run `$opsxx-reconcile-tasks <name>`.
...
5. **Reverse-sync implementation truth**

   If `newChange` is `true`, skip this step: review and verification own any disagreement introduced by the first apply.

   Otherwise, run `$opsxx-reconcile-specs <name>` and resolve implementation/spec drift before review.
