...
The argument after $opsxx-reconcile-tasks is the change name (kebab-case). If omitted, ask the user after listing changes via `openspec list --json`.

Optional second argument: `--write` to actually edit tasks.md. Default is dry-run (report only).
...
6. **If `--write` was supplied**: for every task classified plain `done-silently` (NOT the `-with-drift` or `-with-semantic-drift` sub-variants), edit tasks.md to:
   - Change `- [ ]` to `- [x]`
...
- **Before $opsx-apply <change>**: run $opsxx-reconcile-tasks first to avoid re-doing landed work.
