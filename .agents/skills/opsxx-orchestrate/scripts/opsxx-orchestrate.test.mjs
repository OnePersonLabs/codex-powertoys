...
test('codex worker descriptors live under CODEX_HOME worktrees', () => {
  ...
  assert.equal(
    ...
    '/tmp/codex-home/worktrees/psychord-agent-add-example',
  )
...
test('codex worker launch uses direct argv and cwd with unusual paths', () => {
  ...
  assert.equal(
    launch.cwd,
    '/tmp/codex home [one]/worktrees/psychord-agent-add-example',
  )
...
test('retained worktree capacity fails closed without deleting evidence', () => {
  ...
  try {
    for (let i = 0; i < 16; i++) {
      const dir = path.join(
        worktreeRoot,
        `psychord-agent-old-${String(i).padStart(2, '0')}`,
      )
    ...
    const descriptor = {
      change: 'new-change',
      worktreeRoot,
      worktreePath: path.join(worktreeRoot, 'psychord-agent-new-change'),
    ...
    assert.equal(
      fs
        ...
        .filter((name) => name.startsWith('psychord-agent-')).length,
      16,
    )
  } finally {
    fs.rmSync(root, { recursive: true, force: true })
...
test('preflight rejects wrong branch, missing change folder, and dirty worktree', () => {
  const descriptor = {
    ...
    branch: 'orchestrate/add-example',
    worktreeRoot: '/tmp/codex-home/worktrees',
    worktreePath: '/tmp/codex-home/worktrees/psychord-agent-add-example',
  }
