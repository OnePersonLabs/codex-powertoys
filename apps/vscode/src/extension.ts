...
import { discoverAgents, discoverMcps, discoverSkills, loadMcpTools, setMcpState, setSkillEnabled, type AgentRecord, type McpRecord, type SkillRecord, type SupportingEntry } from "@codex-inspector/core";
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  command("codexInspector.mcp.add", async () => { const name = await vscode.window.showInputBox({ prompt: "MCP name" }); const raw = await vscode.window.showInputBox({ prompt: "MCP config as JSON" }); if (!name || !raw) return; const { addMcpDefinition } = await import("@codex-inspector/core"); await addMcpDefinition({ workspaceRoot: vscode.workspace.workspaceFolders?.[0]?.uri.fsPath }, "workspace", name, JSON.parse(raw)); await mcps.refresh(); });
  command("codexInspector.mcp.edit", async (mcp: McpRecord) => { const raw = await vscode.window.showInputBox({ prompt: "MCP patch as JSON", value: JSON.stringify(mcp.config) }); if (!raw) return; const { updateMcpDefinition } = await import("@codex-inspector/core"); await updateMcpDefinition({ workspaceRoot: vscode.workspace.workspaceFolders?.[0]?.uri.fsPath }, mcp.scope, mcp.name, JSON.parse(raw)); await mcps.refresh(); });
  command("codexInspector.mcp.delete", async (mcp: McpRecord) => { if (await vscode.window.showWarningMessage(`Delete MCP ${mcp.name}?`, { modal: true }, "Delete") !== "Delete") return; const { deleteMcpDefinition } = await import("@codex-inspector/core"); await deleteMcpDefinition({ workspaceRoot: vscode.workspace.workspaceFolders?.[0]?.uri.fsPath }, mcp.scope, mcp.name); await mcps.refresh(); });
