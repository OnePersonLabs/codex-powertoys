...
test("applies workspace-over-global plugin MCP policy overlays without changing transport config", async () => {
  ...
  await writeFile(join(values.workspace, ".codex", "config.toml"), `[plugins."plugin@source".mcp_servers.pluginServer]\ndisabled_tools = ["safe"]\n`);
  const result = await discoverMcps({ homeDir: values.home, codexHome: values.codex, workspaceRoot: values.workspace });
  const pluginMcp = result.mcps.find((record) => record.name === "pluginServer");
...
function mcpRecord(config: Record<string, unknown>): McpRecord {
  return {
    id: "test:mcp", name: "test", scope: "global", sourceKind: "config", configPath: "test.toml", config,
    enabled: true, pluginEnabled: true, effective: "active", sourceRange: { path: "test.toml", startLine: 0, endLine: 0 }, diagnostics: []
  };
}
...
test("derives MCP tool permission glyphs from deny/allow lists and approval overrides", async () => {
  const transport: McpTransport = {
    async request(method) {
      if (method === "tools/list") return { tools: [{ name: "allowed" }, { name: "denied" }, { name: "missing" }, { name: "prompted" }] };
      return undefined;
    },
    async close() {}
  };
  const result = await loadMcpTools(mcpRecord({
    enabled_tools: ["allowed", "denied", "prompted"], disabled_tools: ["denied"], default_tools_approval_mode: "auto",
    tools: { prompted: { approval_mode: "prompt" } }
  }), { transport });
  assert.deepEqual(result.tools.map((tool) => [tool.name, tool.permissionGlyph]), [
    ["allowed", "✅"], ["denied", "❌"], ["missing", "❌"], ["prompted", "✋"]
  ]);
});
...
test("does not wait for JSON-RPC notifications and reports JSON-RPC errors", async () => {
  ...
  const transport: McpTransport = {
    request(method) {
      methods.push(method);
      if (method === "notifications/initialized") return new Promise<never>(() => {});
  ...
  const result = await Promise.race([
    ...
    new Promise<never>((_, reject) => setTimeout(() => reject(new Error("notification hung")), 250))
  ]);
...
test("applies the probe timeout to injected transports that never answer", async () => {
  const transport: McpTransport = { request: async () => new Promise<never>(() => {}), close: async () => {} };
  const result = await loadMcpTools(mcpRecord({}), { transport, timeoutMs: 20 });
...
test("terminates owned stdio MCP children after a successful probe", async () => {
  const pidPath = join(tmpdir(), `codex-mcp-pid-${Date.now()}-${Math.random()}.txt`);
  const script = [
  ...
  const result = await loadMcpTools(mcpRecord({ command: process.execPath, args: ["-e", script] }), { timeoutMs: 1000 });
...
test("reports an early stdio child exit instead of hanging the probe", async () => {
  const result = await loadMcpTools(mcpRecord({ command: process.execPath, args: ["-e", "process.exit(7)"] }), { timeoutMs: 200 });
  assert.equal(result.tools.length, 0);
  assert.match(result.diagnostics[0]?.message ?? "", /exited|closed|unavailable/);
});
